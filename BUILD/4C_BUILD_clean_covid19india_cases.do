* Clean Covid19India API case data

global dirpath_covid = "$dirpath/data/Generated/Intermediate/Covid"
global dirpath_district_names = "$dirpath/data/Generated/Intermediate/Shapefiles/districts/aggregated"

* Load data from covid19india json api
import delimited "$path_covid/covid19india_api.csv", clear
...
* Fix names to better match 2019 shapefile
...
* Make district id for matching to 2019 shapefile and linking back to 
* full time series
...
* Keep only necessary variables/observations for matching to 2019 shapefile
...
* Load names from 2018 district shapefile
use "$dirpath_district_names/districts_delhiagg.dta", clear
...
* Fuzzy join 2019 shapefile and cases time series
...
* Use 2019 district shapefile names
drop district state
...
************************************************************************
** Back fill negative cases, deaths, and tests over previous positive entries
...
************************************************************************
* For when a case is reported before a test, evenly fill the first reported
* testing value across all days from when the test was first reported
* and 1 day prior to the first case being reported
...
replace num_tested = 0 if missing(num_tested)
by state district: egen p99 = pctile(num_tested), p(99)
replace num_tested = p99 if num_tested > p99
...
* Replace lagged weekly sum value to missing if no tests are reported past a 
* given day in the sample
...
* Average deaths on june 16 (reporting spike) over the previous 7 days for
* delhi and maha
...
* Output
save "$dirpath_covid/covid19india_data_cleaned.dta", replace
